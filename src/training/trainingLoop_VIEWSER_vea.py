...
from HydraBNrecurrentUnet_01_vae import HydraBNUNet01_vae
...
def make(config):
    ...
    elif config.model == 'HydraBNUNet01_vae':
        unet = HydraBNUNet01_vae(config.input_channels, config.hidden_channels, config.output_channels, config.dropout_rate).to(device)
...
def train(model, optimizer, scheduler, criterion_reg, criterion_class, multitaskloss_instance, train_tensor, config, device):
    ...
    for i in range(seq_len-1): # so your sequnce is the full time len - last month.
        ...
        # forward
        t1_pred, t1_pred_class, h, mu, sigma = model(t0, h.detach())
        ...
        reconst_loss = multitaskloss_instance(losses)
...
def test(model, test_tensor, time_steps, config, device): # should be called eval/validation
    ...
    for i in range(seq_len-1): # need to get hidden state... You are predicting one step ahead so the -1
        ...
        if i < seq_len-1-time_steps: # take form the test set
            ...
            t0 = test_tensor[:, i, :, :, :]
            t1_pred, t1_pred_class, h_tt, _, _ = model(t0, h_tt)
            ...
            t1_pred, t1_pred_class, _, _, _ = model(t0, h_tt) # freeze
