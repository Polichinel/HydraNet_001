...
class HydraBNUNet06_LSTM_PE(nn.Module):
    def __init__(self, input_channels, hidden_channels, output_channels, dropout_rate):
        ...
        # REMOVE HARD CODING 5 IF WORKS amd but it in config!!!!!!!!!!!!!

        # Input gate
        self.Wxi = nn.Conv2d(input_channels+5, hidden_channels_split, kernel_size, padding=padding, bias=True) # if it runs, try to remove bias - you are using batchnorm after all
        self.Whi = nn.Conv2d(hidden_channels_split, hidden_channels_split, kernel_size, padding=padding, bias=True)
        self.Wxf = nn.Conv2d(input_channels+5, hidden_channels_split, kernel_size, padding=padding, bias=True)
        self.Whf = nn.Conv2d(hidden_channels_split, hidden_channels_split, kernel_size, padding=padding, bias=True)
        self.Wxc = nn.Conv2d(input_channels+5, hidden_channels_split, kernel_size, padding=padding, bias=True)
        self.Whc = nn.Conv2d(hidden_channels_split, hidden_channels_split, kernel_size, padding=padding, bias=True)
        self.Wxo = nn.Conv2d(input_channels+5, hidden_channels_split, kernel_size, padding=padding, bias=True)
    ...
    def pos_embd_tensor(pe_channels = 5, scale_par = 1, H = 180, W = 180):
        ...
        """
        pos_emb_tensor = torch.zeros((1,pe_channels, H, W), dtype= torch.float64) # is this on device?
    ...
    def forward(self, x, h):
        ...
        # Input gate
        i_t = torch.sigmoid(self.Wxi(x_pet) + self.Whi(hs)) # Wxi changes to dims for x to the same as hs

        # Forget gate
        f_t = torch.sigmoid(self.Wxf(x_pet) + self.Whf(hs)) # Wxf changes to dims for x to the same as hs

        # Cell state
        hl_tilde = torch.tanh(self.Wxc(x_pet) + self.Whc(hs)) # Wxc changes to dims for x to the same as hs
        hl = f_t * hl + i_t * hl_tilde

        # Output gate
        o_t = torch.sigmoid(self.Wxo(x_pet) + self.Who(hs)) # Wxo changes to dims for x to the same as hs
