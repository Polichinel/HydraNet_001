...
class HydraBNUNet04(nn.Module):
    def __init__(self, input_channels, hidden_channels, output_channels, dropout_rate):
        ...
        # HEAD1
        self.upsample0_head1 = nn.ConvTranspose2d(base*4, base*2, 2, stride= 2, padding= 0, output_padding= 0) # 4 -> 8
        self.dec_conv0_head1 = nn.Conv2d(base*4, base*2, 3, padding=1, bias = False) # base+base=base*2 because of skip conneciton
        self.bn_dec_conv0_head1 = nn.BatchNorm2d(base*2) 

        self.upsample1_head1 = nn.ConvTranspose2d(base*2, base, 2, stride= 2, padding= 0, output_padding= 0) # 8 -> 16
        self.dec_conv1_head1 = nn.Conv2d(base*2, base, 3, padding=1, bias = False) # base+base=base*2 because of skip connection
        self.bn_dec_conv1_head1 = nn.BatchNorm2d(base) 
        ...
        # HEAD2
        self.upsample0_head2 = nn.ConvTranspose2d(base*4, base*2, 2, stride= 2, padding= 0, output_padding= 0) # 4 -> 8
        self.dec_conv0_head2 = nn.Conv2d(base*4, base*2, 3, padding=1, bias = False) # base+base=base*2 because of skip conneciton
        self.bn_dec_conv0_head2 = nn.BatchNorm2d(base*2) 

        self.upsample1_head2 = nn.ConvTranspose2d(base*2, base, 2, stride= 2, padding= 0, output_padding= 0) # 8 -> 16
        self.dec_conv1_head2 = nn.Conv2d(base*2, base, 3, padding=1, bias = False) # base+base=base*2 because of skip connection
        self.bn_dec_conv1_head2 = nn.BatchNorm2d(base) 
        ...
        # HEAD3
        self.upsample0_head3 = nn.ConvTranspose2d(base*4, base*2, 2, stride= 2, padding= 0, output_padding= 0) # 4 -> 8
        self.dec_conv0_head3 = nn.Conv2d(base*4, base*2, 3, padding=1, bias = False) # base+base=base*2 because of skip conneciton
        self.bn_dec_conv0_head3 = nn.BatchNorm2d(base*2) 

        self.upsample1_head3 = nn.ConvTranspose2d(base*2, base, 2, stride= 2, padding= 0, output_padding= 0) # 8 -> 16
        self.dec_conv1_head3 = nn.Conv2d(base*2, base, 3, padding=1, bias = False) # base+base=base*2 because of skip connection
        self.bn_dec_conv1 = nn.BatchNorm2d(base) 
    ...
    def forward(self, x, h):
        ...
        #H1
        H1_d0 = F.relu(self.bn_dec_conv0_head1(self.dec_conv0_head1(torch.cat([self.upsample0_head1(b),e1s],1))))
        H1_d0 = self.dropout(H1_d0)
        
        H1_d1 = F.relu(self.bn_dec_conv1_head1(self.dec_conv1_head1(torch.cat([self.upsample1_head1(H1_d0), e0s],1)))) # You did not have any activations before - why not?
        H1_d1 = self.dropout(H1_d1)

        H1_d2 = F.relu(self.bn1_head1(self.dec_conv2_pre_head1(H1_d1)))
        H1_d2 = self.dropout(H1_d2) # is this good?

        H1_reg = F.relu(self.bn2_head1(self.dec_conv2_reg1_head1(H1_d2)))
        H1_reg = self.dropout(H1_reg) # is this good?
        H1_reg = self.dec_conv2_reg2_head1(H1_reg)
        
        H1_class = F.relu(self.bn3_head1(self.dec_conv2_class1_head1(H1_d2)))
        ...
        H2_d1 = F.relu(self.bn_dec_conv1(self.dec_conv1_head2(torch.cat([self.upsample1_head2(H2_d0), e0s],1)))) # You did not have any activations before - why not?
        H2_d1 = self.dropout(H2_d1)

        H2_d2 = F.relu(self.bn1_head2(self.dec_conv2_pre_head2(H2_d1)))
        H2_d2 = self.dropout(H2_d2)

        H2_reg = F.relu(self.bn2_head2(self.dec_conv2_reg1_head2(H2_d2)))
        H2_reg = self.dropout(H2_reg)
        H2_reg = self.dec_conv2_reg2_head2(H2_reg)

        H2_class = F.relu(self.bn3_head2(self.dec_conv2_class1_head2(H2_d2)))
        ...
        d1 = F.relu(self.bn_dec_conv1(self.dec_conv1_head3(torch.cat([self.upsample1_head3(d0), e0s],1)))) # You did not have any activations before - why not?
        d1 = self.dropout(d1)
