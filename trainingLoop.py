...
# New:
criterion_reg = geomloss.SamplesLoss(loss='sinkhorn', scaling = 0.3, reach = 10, backend = 'multiscale', p = 2, blur= 0.05, verbose=False).to(device)
criterion_class = geomloss.SamplesLoss(loss='sinkhorn', scaling = 0.3, reach = 10, backend = 'multiscale', p = 2, blur= 0.05, verbose=False).to(device)
